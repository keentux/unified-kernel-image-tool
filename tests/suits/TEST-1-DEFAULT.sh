...
_verify_boot() {
    ...
    test_run_cmd_vm "${UKITOOL_PATH}" grub2 --add \
        -u "${TEST_CREATE_UKI_PATH}" -D -t "UKI test"
    test_run_reboot
}
...
#    TEST_UNAME: kernel version for the test
# OUTPUTS:
#   None
# RETURN:
...
###
test_clear() {
    [ -f "${TEST_CREATE_UKI_PATH}" ] && rm "${TEST_CREATE_UKI_PATH}"
    [ -f "${TEST_CREATE_INITRD_PATH}" ] && rm "${TEST_CREATE_INITRD_PATH}"
}