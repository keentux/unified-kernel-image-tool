...
#   None
#   
function _grub2_remove_menuentry() {
    local grub_config_path="$1"
    local file_path="$2"
    local file
    ...
    if [[ -f "$grub_config_path" ]]; then
        if grep -q "$file_path" "$grub_config_path"; then
            ...
            done < <(grep -n "EOF" "$grub_config_path")
...
#   2
###
function _grub2_usage() {
    echo -e "$0 grub2 [--add-entry | --remove-entry] [-k | --kerver] [-i | --initrd ] [ -u | --uki]
...
example:
    $0 grub2 --add-entry -k 6.3.4-1-default -u /boot/efi/EFI/opensuse/uki.efi\n"
}
...
function _grub2_initrd() {
    ...
    if [[ $cmd -eq $GRUB2_CMD_ADD ]]; then
        ...
        if [[ ! -f /boot/$initrd_file ]]; then
            echo_info "$initrd_file isn't in boot partition, copy it to /boot/$initrd_file"
...
function _grub2_uki() {
    ...
    if [[ $cmd -eq $GRUB2_CMD_ADD ]]; then
        ...
        if [[ ! -f /boot/efi/EFI/opensuse/${uki_file} ]]; then
            echo_info "$uki_file isn't in efi partition, copy it to /boot/efi/EFI/opensuse/$uki_file"
...
function grub2_exec() {
    ...
    do
        case "$1" in
            --add-entry)        local cmd_add=1         ; shift 1 ;;
            --remove-entry)     local cmd_remove=1      ; shift 1 ;;
            -k | --kerver)      local kerver="$2"       ; shift 2 ;;
            -i | --initrd)      local initrd_path="$2"  ; shift 2 ;;
            -u | --uki)         local uki_path="$2"     ; shift 2 ;;
            --)                 shift                   ; break   ;;
            *)                  echo_warning "Unexpected option: $1"; usage   ;;
        esac
    done
    # Chack the command
    if [[ -z $cmd_add && -z $cmd_remove ]]; then
        echo_error "Need \"add-entry\" or \"remove-entry\" command"
        _grub2_usage
        exit 2
    elif [[ $cmd_add -eq 1 && $cmd_remove -eq 1 ]]; then
        echo_error "Please choose between add or remove a menue entry. Not both!"
        _grub2_usage
        exit 2
    elif [[ $cmd_add -eq 1 ]]; then
        cmd=$GRUB2_CMD_ADD
    else
